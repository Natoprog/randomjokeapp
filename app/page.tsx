      ...
      <div className="container">
        <header>
          ...
          <div className="app-theme">
            <button className="toggle-btn" onClick={handleChangeTheme}>
              {!isDark ? (
                <DarkModeIcon fontSize="large" sx={{ color: "black" }} />
              ) : (
        ...
        <div className="container-inner">
          {loading ? (
            ...
            <>
              {data.map((joke, i) => (
                <h3 key={i} className="question">
                  {joke.setup}
                </h3>
              ))}
              {data.map((joke, i) => (
                <h4 key={i} className="punchline">
                  {joke.punchline}
                </h4>
              ))}
            </>
          )}
          <button className="next-btn" onClick={getData}>
            Next Joke
          </button>
        </div>
        ...
        <footer>
          ...
          <Link
            href="https://github.com/Natoprog/randomjokeapp"
            className="footer-link"
            target="_blank"
          >
            <GitHubIcon sx={{ color: isDark ? "white" : "black" }} />
          </Link>
        </footer>
      </div>
    </>
